...
class FaviconLoad {
    ...
    constructor(iconInfo) {
        ...
        this.channel = Services.io.newChannelFromURI(iconInfo.iconUri, iconInfo.node, iconInfo.node.nodePrincipal, iconInfo.node.nodePrincipal, securityFlags |
            Ci.nsILoadInfo.SEC_ALLOW_CHROME |
            Ci.nsILoadInfo.SEC_DISALLOW_SCRIPT, Ci.nsIContentPolicy.TYPE_INTERNAL_IMAGE_FAVICON);
    ...
    async onStopRequest(request, statusCode) {
        ...
        if (this.channel instanceof Ci.nsIHttpChannel) {
            if (!this.channel.requestSucceeded) {
                this._deferred.reject(Components.Exception(`Favicon at "${this.icon.iconUri.spec}" failed to load: ${this.channel.responseStatusText}.`, { data: { httpStatus: this.channel.responseStatus } }));
                return;
            }
        }
        ...
        try {
            ...
            if (type != 'image/svg+xml') {
                ...
                let sniffer = Cc['@mozilla.org/image/loader;1'].createInstance(Ci.nsIContentSniffer);
...
function selectIcons(iconInfos, preferredWidth) {
    ...
    for (let icon of iconInfos) {
        if (!icon.isRichIcon) {
            ...
            else if (icon.width == preferredWidth &&
                guessType(preferredIcon) != TYPE_SVG) {
                preferredIcon = icon;
            }
            else if (guessType(icon) == TYPE_ICO &&
...
class IconLoader {
    ...
    async load(iconInfo) {
        ...
        if (LOCAL_FAVICON_SCHEMES.includes(iconInfo.iconUri.scheme)) {
            ...
            // it for us.
            try {
                Services.scriptSecurityManager.checkLoadURIWithPrincipal(iconInfo.node.nodePrincipal, iconInfo.iconUri, Services.scriptSecurityManager.ALLOW_CHROME);
            }
            catch (ex) {
                return;
            }
...
export class FaviconLoader {
    ...
    constructor(actor) {
        ...
        this.iconTask = new lazy.DeferredTask(() => this.loadIcons(), FAVICON_PARSING_TIMEOUT);
    }
    loadIcons() {
        ...
        let preferredWidth = PREFERRED_WIDTH * Math.ceil(this.actor.contentWindow.devicePixelRatio);
